            ...
            if (["mp3", "wav", "ogg", "flac", "aac", "m4a", "wma", "aiff", "alac", "ape", "opus", "dsd"].includes(getFileExtension(library.file_upload))) {
              return (
                <div key={index} className="flex items-start gap-2.5">
                  <img className="w-8 h-8 rounded-full" src={library.author.image} alt="Jese image" />
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center space-x-2 rtl:space-x-reverse">
                      <span className="text-sm font-semibold text-gray-900 dark:text-white">{`${library.author.first_name} ${library.author.last_name}`}</span>
                      <span className="text-sm font-normal text-gray-500 dark:text-gray-400">{new Date(library.created_at).toLocaleDateString()}</span>
                    </div>
                    <div className="flex flex-col w-[400px] leading-1.5 py-2 rounded-e-xl rounded-es-xl">
                      <div className="flex items-center space-x-2 rtl:space-x-reverse">
                        <button className="inline-flex self-center items-center p-2 text-sm font-medium text-center text-gray-900 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:hover:bg-gray-600 dark:focus:ring-gray-600" type="button" onClick={() => window.open(library.file_upload, "_blank").focus()}>
