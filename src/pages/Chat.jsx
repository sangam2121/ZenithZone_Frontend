...
const Chat = () => {
   ...
   const firstTimeChatOtherUserId=location.state?.otherUserId;
 
   const firstTimeChat = async () => {
      try {
         const response = await fetch(`${import.meta.env.VITE_AUTH_BASE_URL}/doctor/update/${firstTimeChatOtherUserId}`, {
            method: "GET",
         });
         ...
         if (response.ok) {
            ...
            const newUser = {
               ...
               otherUserId: firstTimeChatOtherUserId,
   ...
   const fetchChatroom = async () => {
      try {
         ...
         const response = await fetch(`${import.meta.env.VITE_AUTH_BASE_URL}/chat/chatrooms/`, {
            method: "GET",
            headers: {
         ...
         if (response.ok) {
            ...
            setChatRoom(formattedChatRooms);
         } else {
            console.log("Error fetching chat rooms:", response.status);
         }
      } catch (error) {
   ...
   useEffect(() => {
      if (otherUserId) {
         // Initialize WebSocket connection when otherUserId is set
         const newSocket = new WebSocket(`ws://localhost:8000/ws/chat/${userId}/${otherUserId}`);
      ...
      // Ensure WebSocket connection is closed when otherUserId changes to null
      if (!otherUserId && socket) {
         socket.close();
         setSocket(null); // Reset socket state
         console.log('WebSocket connection closed');
      }
   }, [userId, otherUserId]); // Depend on userId and otherUserId
   ...
   const handleUserClick = async (user) => {
      try {
         const { otherUserId, chatId } = user;
         setOtherUserId(otherUserId);
         setSelectedUser(user);
         setMessages([]); // Clear messages when a new user is selected

         const response = await fetch(`${import.meta.env.VITE_AUTH_BASE_URL}/chat/messages/?chat_room=${chatId}`, {
            method: "GET",
            headers: {
               'Authorization': `Bearer ${localStorage.getItem("access")}`
            }
      ...
      <>
         ...
         <div className='h-[88vh] overflow-y-auto flex'>
            ...
            <div className='w-1/4 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700'>
               <div className='p-4'>
                  ...
                  <ul className='my-4 space-y-3'>
                     {
                        chatRoom && chatRoom.map((user) => {
                           return (
                              <>
                                 <li>
                                    <a href='#' className='flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white overflow-x-hidden' onClick={() => handleUserClick(user)}>
                                       <div className='flex items-center'>
                                          ...
                                          <div className='flex-1 min-w-0 ms-4'>
                                             ...
                                             <p className='text-sm text-gray-500 truncate dark:text-gray-400'>
                                                {`${user.lastMessage.username}: ${user.lastMessage.content}`}
                                             </p>
                                          </div>
                                       </div>
                                    </a>
            ...
            <div className="flex-1 bg-white">
               {
                  selectedUser ? (<div id="messages" className="flex flex-col space-y-4 p-3 overflow-y-hidden git  h-full bg-white border border-gray-200 rounded-lg shadow me-2">
                     <div class="">
                        <div class="flex items-center">
                           <div class="flex-shrink-0">
                              <div className='relative'>
                                 <img className='w-12 h-12 rounded-full' src={`http://localhost:8000${selectedUser.image}`} alt='' />
                                 <span className='top-0 left-7 absolute w-3.5 h-3.5 bg-green-400 border-2 border-white dark:border-gray-800 rounded-full'></span>
                              </div>
                           </div>
                     ...
                     <div ref={chatContainerRef} className="w-full h-[90%] p-4 rounded-lg sm:p-6 relative overflow-y-scroll">
                        {/* Sender's message */}
                              ...
                              return (
                                 <>
                                    {!isSentByMe ? (<div className="flex items-end gap-2.5 mb-4">
                                       ...
                                       <div key={index} className="flex items-end justify-end gap-2.5 mb-4">
                     ...
                     {/* Form */}
                     <form onSubmit={sendMessage} className="w-full  bg-gray-50 dark:bg-gray-700">
                        ...
                        <div class="flex items-center px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-700">
                           ...
                           <input type="text"
                              ...
                              id="chat" rows="1" class="block mx-4 p-2.5 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" ></input>
